...
2. Copy the `.joblib` files into this folder (`backend/models_back/`).
3. Set `MODEL_APARTMENT` / `MODEL_HOUSE` in `backend/.env` if filenames differ from defaults.
...
docker run --rm \
  -v VOLUME_NAME:/dest \
  -v /tmp:/tmp \
  alpine sh -c "cp /tmp/*.joblib /dest/ && ls -lh /dest/"
```

Alternative via mount point:

```bash
sudo cp /tmp/*.joblib /var/lib/docker/volumes/VOLUME_NAME/_data/
sudo ls -lh /var/lib/docker/volumes/VOLUME_NAME/_data/
```

You should see both `.joblib` files plus optionally `README.md` from the image build.
...
2. Update `MODEL_APARTMENT` / `MODEL_HOUSE` in Coolify **Production** env if filenames change.
3. Repeat steps 1–4 (SCP → volume → redeploy).
4. Old `.joblib` files can be removed from the volume to save disk space.
